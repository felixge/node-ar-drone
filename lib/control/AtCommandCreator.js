...
AtCommandCreator.prototype.raw = function(type, args, blocks, options) {
  args = (Array.isArray(args))
    ? args
    : Array.prototype.slice.call(arguments, 1);

  return new AtCommand(type, args, blocks, options);
};

AtCommandCreator.prototype.ctrl = function(controlMode) {
    var args = [controlMode, 0];
...
AtCommandCreator.prototype.config = function(name, value, options) {
  return this.raw('CONFIG', ['"' + name + '"', '"' + value + '"'], true, options);
};
