...
function UdpControl(options) {
  ...
  this.defaultRetries = options.defaultRetries || DEFAULT_NUM_RETRIES;
...
UdpControl.prototype.ack = function(ack) {
  if (this._blocked.state === states.WAITING_FOR_ACK) {
    ...
    if (this._blocked.cmd.options.callback) {
      this._blocked.cmd.options.callback.call();
    }
...
UdpControl.prototype._handleAckTimeout = function() {
  ...
  if (this._blocked.cmd.options.callback) {
    this._blocked.cmd.options.callback.call(null, new Error('ACK Timeout'));
  }
  this._blocked.cmd = null;
  this._blocked.state = states.NOT_BLOCKED;
};
