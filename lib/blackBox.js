...
Blackbox.prototype.writeNavdata = function() {
  ...
  this._writeTsv(this.navdata, this.navdataFile);
};

Blackbox.prototype.writeControl = function() {
  this.controlFile = fs.createWriteStream(this.path + '/control.tsv');
  this._writeTsv(this.control, this.controlFile);
};

Blackbox.prototype._writeTsv = function(source, destination) {
  var first = true;
  source.on('data', function(data) {
    data = flattenObject(data);

    if (first) {
      var header = ['time'].concat(Object.keys(data));
      destination.write(header.join('\t') + '\n');
      first = false;
    }

    destination.write([Date.now()].concat(_.values(data)).join('\t') + '\n');
  });
};
